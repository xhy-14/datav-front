  ...
  <el-cotainer style="height: 98vh; display: flex; justify-content: center;align-items: center;">
    <el-form ref="formRef" :model="form" label-width="120px" style="margin-right: 10%; margin-left:10%; width: 50%; ">
      ...
      <el-form-item prop="ip" label="数据库ip" :rules="{ required: true, message: '请输入数据库ip', trigger: 'blur' }">
        <el-input v-model="form.ip" placeholder="" />
      </el-form-item>
      ...
      <el-form-item prop="databaseName" label="数据库名称" :rules="{ required: true, message: '请输入数据库名称', trigger: 'blur' }">
        <el-input v-model="form.databaseName" placeholder="" />
      </el-form-item>
      ...
      <el-form-item prop="attribute" label="连接属性">
        <template #default>
          <el-table :data="form.tableData" style="width: 100%" max-height="250">
            <el-table-column prop="key" label="属性名" width="auto">
              <template #default="scope">
                <el-input ref="inputRef" class="w-50 m-2" size="small" @blur="toEdit(scope.row, scope.$index)"
                  v-if="state.tableColumnEditIndex == scope.column.id && state.tableRowEditIndex === scope.$index"
                  placeholder="请输入" v-model="scope.row.key" />
                <div v-else @click="toEditDiv(scope.row, scope.column, scope.$index)">
                  {{ scope.row.key }}
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column fixed="right" v-if="form.tableData.length != 0" label="操作" width="120">
              <template #default="scope">
          ...
          <el-button class="mt-4" style="width: 100%" @click="addRow()">添加</el-button>
        </template>
      </el-form-item>
      <el-form-item>
